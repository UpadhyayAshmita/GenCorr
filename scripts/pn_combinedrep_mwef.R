...
pn_blues <- read.csv(paste0("pn_blues_rep", repetition, ".csv"))
...
run_trait_model <- function(trait, trait_index, cv_scheme) {
  ac_rep <- c()
  fname <- paste0("pnW", trait_index, "_", cv_scheme, "_rep", repetition, "_mwef.txt")
  ac_fname <- paste0("acpnW", trait_index, "_", cv_scheme, "_rep", repetition, "_mwef.txt")
