...
# CHANGED: folds created from MW (instead of EF)
...
fwrite(data.frame(taxa = character(), GEBV = numeric()),
       "NT_CV1_MWEF.csv", sep = ",", col.names = TRUE)
...
for(j in 1:length(sort)){
  ...
  fwrite(raN, "NT_CV1_MWEF.csv", sep = ",", append = TRUE, col.names = FALSE)

  # CHANGED: evaluate on EF (instead of MW)
  raN_eval <- raN %>% left_join(N_bluesEF1[, c("taxa", "narea")])
...
fwrite(data.frame(rep = 1:length(acNT_CV1), acc = acNT_CV1),
       "acNT_CV1_MWEF.csv", sep = ",", col.names = TRUE)
...
# init CSV with header
fwrite(data.frame(taxa = character(), GEBV = numeric()),
       "NT_CV2_MWEF.csv", sep = ",", col.names = TRUE)
...
for(j in 1:length(sort)){
  ...
  # append to CSV (no header)
  fwrite(raN, "NT_CV2_MWEF.csv", sep = ",", append = TRUE, col.names = FALSE)
...
fwrite(data.frame(rep = 1:length(acNT_CV2), acc = acNT_CV2),
       "acNT_CV2_MWEF.csv", sep = ",", col.names = TRUE)
