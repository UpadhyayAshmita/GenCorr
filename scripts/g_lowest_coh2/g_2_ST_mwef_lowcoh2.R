...
# CHANGED: folds now created from MW taxa
...
fwrite(data.frame(taxa = character(), GEBV = numeric()),
       "ST_CV1_MWEF.csv", sep = ",", col.names = TRUE)
...
for(j in 1:length(sort)){
  ...
  fwrite(raS, "ST_CV1_MWEF.csv", sep = ",", append = TRUE, col.names = FALSE)

  # CHANGED: evaluate on EF
  raS_eval <- raS %>% left_join(S_bluesEF1[, c("taxa", "sla")])
...
fwrite(data.frame(rep = 1:length(acST_CV1), acc = acST_CV1),
       "acST_CV1_MWEF.csv", sep = ",", col.names = TRUE)
...
# init CSV with header
fwrite(data.frame(taxa = character(), GEBV = numeric()),
       "ST_CV2_MWEF.csv", sep = ",", col.names = TRUE)
...
for(j in 1:length(sort)){
  ...
  # append to CSV (no header)
  fwrite(raS, "ST_CV2_MWEF.csv", sep = ",", append = TRUE, col.names = FALSE)
...
fwrite(data.frame(rep = 1:length(acST_CV2), acc = acST_CV2),
       "acST_CV2_MWEF.csv", sep = ",", col.names = TRUE)
