...
# CHANGED: folds now created from MW (instead of EF)
sort <- create_folds(individuals = N_bluesMW$taxa, nfolds = 5,
                     reps = 20, seed = 123)
...
fwrite(data.frame(taxa = character(), GEBV = numeric()),
       "pnT_CV1_MWEF.csv", sep = ",", col.names = TRUE)

for(j in 1:length(sort)){
  rpnT <- list()
  for(i in 1:5){
    # CHANGED: train on MW (mask in MW)
    test <- pnT_bluesMW1
  ...
  fwrite(ra, "pnT_CV1_MWEF.csv", sep = ",", append = TRUE, col.names = FALSE)

  # CHANGED: evaluate on EF (instead of MW)
  ra_eval <- ra %>% left_join(pnT_bluesEF1[, c("taxa", response_trait)])
...
fwrite(data.frame(rep = 1:length(acpnT_CV1), acc = acpnT_CV1),
       "acpnT_CV1_MWEF.csv", sep = ",", col.names = TRUE)
...
# init CSV with header
fwrite(data.frame(taxa = character(), GEBV = numeric()),
       "pnT_CV2_MWEF.csv", sep = ",", col.names = TRUE)
...
for(j in 1:length(sort)){
  ...
  # append to CSV (no header)
  fwrite(ra, "pnT_CV2_MWEF.csv", sep = ",", append = TRUE, col.names = FALSE)
...
fwrite(data.frame(rep = 1:length(acpnT_CV2), acc = acpnT_CV2),
       "acpnT_CV2_MWEF.csv", sep = ",", col.names = TRUE)
