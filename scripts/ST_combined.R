...
# Load and prepare sla_blues data
S_blues <- read.csv(paste0("S_blues1_rep", repetition, ".csv"))
...
run_trait_model <- function() {
  ac_rep <- c()
  fname <- paste0("ST", repetition, "_", cv_scheme, "_rep", repetition, ".txt")
  ac_fname <- paste0("acST", repetition, "_", cv_scheme, "_rep", repetition, ".txt")
  ...
  for (j in 1:length(sort)) {
    ...
    raS <- raS %>% left_join(S_bluesMW[, c("taxa", "sla")])
