...
# Note: Using your established logic
...
# --- 4. Load Phenotypes ---
# File: ps_blues.csv | Trait inside: plsr_sla
...
# --- 5. Get Starting Values (Flipped to MW) ---
cat(sprintf("\nStep 1: Fitting full model on MW for %s (plsr_sla + Waves)...\n", cv_scheme))
...
fname <- paste0("MT_ps_3Trait_", cv_scheme, "_preds_mwef.csv")
ac_fname <- paste0("MT_ps_3Trait_", cv_scheme, "_accuracy_mwef.csv")
...
for (j in 1:20) {
  ...
  for (i in 1:5) {
    ...
    test_data <- ps_MW # Training on MW

    # MASKING LOGIC (Using plsr_sla)
    if (cv_scheme == "CV1") {
  ...
  raS <- bind_rows(r_rep) %>%
    left_join(ps_EF[, c("taxa", "plsr_sla")], by = "taxa") %>%
    rename(obs = plsr_sla)
