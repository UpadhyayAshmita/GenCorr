...
run_trait_model <- function(trait, trait_index, cv_scheme) {
  ...
  fname    <- paste0("pnW", trait_index, "_", cv_scheme, "_MWEF.csv")
  ac_fname <- paste0("acpnW", trait_index, "_", cv_scheme, "_MWEF.csv")
  ...
  for (j in 1:length(sort)) {
    ...
    raS <- raS %>%
      left_join(pn_bluesEF[, c("taxa", "plsr_narea")], by = "taxa") %>%
