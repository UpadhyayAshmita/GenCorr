...
run_trait_model <- function() {
  ac_rep <- c()
  fname <- paste0("NT", repetition, "_", cv_scheme, "_rep", repetition, "_mwef.txt")
  ac_fname <- paste0("acNT", repetition, "_", cv_scheme, "_rep", repetition, "_mwef.txt")
  ...
  for (j in 1:length(sort)) {
    ...
    raS <- raS %>% left_join(N_bluesEF[, c("taxa", "narea")])
