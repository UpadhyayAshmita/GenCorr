...
# Load and prepare N_blues data (kept, only used for folds in your structure)
...
ps_blues <- read.csv("./output/ps_blues.csv")
...
run_trait_model <- function(trait, trait_index, cv_scheme) {
  ...
  fname    <- paste0("psW", trait_index, "_", cv_scheme, "_MWEF.csv")
  ac_fname <- paste0("acpsW", trait_index, "_", cv_scheme, "_MWEF.csv")
  ...
  for (j in 1:length(sort)) {
    ...
    for (i in 1:5) {
      ...
      test <- ps_bluesMW
      test[test$taxa %in% test_ids, "plsr_sla"] <- NA
      ...
      r_rep[[i]] <- pr %>%
        filter(trait == "plsr_sla" & taxa %in% test_ids) %>%
    ...
    raS <- raS %>%
      left_join(ps_bluesEF[, c("taxa", "plsr_sla")], by = "taxa") %>%
      rename(obs = plsr_sla)
