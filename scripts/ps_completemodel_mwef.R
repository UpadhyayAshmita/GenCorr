...
parser$add_argument('--trait1', type = 'character', help = 'First synthetic trait')
parser$add_argument('--trait2', type = 'character', help = 'Second synthetic trait')
parser$add_argument('--trait3', type = 'character', help = 'Third synthetic trait')
...
N_blues <- read.csv(paste0("N_blues.csv"))
...
ps_blues <- read.csv(paste0("./data/ps_blues.csv"))
...
run_trait_model <- function(trait, trait_index, cv_scheme) {
  ac_rep <- c()
  fname <- paste0("psW", trait_index, "_", cv_scheme, "_mwef.txt")
  ac_fname <- paste0("acpsW", trait_index, "_", cv_scheme, "_mwef.txt")
  ...
  for (j in 1:length(sort)) {
    ...
    raS <- raS %>% left_join(ps_bluesEF[, c("taxa", "ps")])
