...
N <- fread("./output/narea_breakdown.csv", data.table = F)
S<- fread("./output/sla_breakdown.csv",data.table=F)
pn<- fread("./output/pn_breakdown.csv",data.table=F)
ps<- fread("./output/ps_breakdown.csv", data.table = F)

wave <- fread("./data/phenotypes_whole.csv", data.table = F) |>
  clean_names()

#for Narea
sum(is.na(N$corg)) #11
sum(N$corg< -1 | N$corg > 1, na.rm = T) #21
...
write.csv(Nratios, "./output/Nratios.csv")
Nratios<- read.csv("./output/Nratios.csv")
...
write.csv(Nratio_transform, "./output/Nratio_transform.csv", row.names=F)
Nratio_transform<- read.csv("./output/Nratio_transform.csv")
...
sel$rowname <- paste0(sel$wave_1, "_", sel$wave_2)
Sratios <- wave |> select(wave_1211_wave_1243:wave_2243_wave_1381)
Sratios <- t(Sratios)
...
write.csv(Sratios, "./output/Sratios.csv")
Sratios<- read.csv("./output/Sratios.csv")
...
Sratio_transform <- Sratio_sel %>%
  mutate(across(c(wave_1, wave_2), ~paste("wave", ., sep = "_"))) %>%
  mutate(wave_sel= paste(wave_1, wave_2, sep = "_")) %>% select(-group,-coh2,-diff)
write.csv(Sratio_transform, "./output/Sratio_transform.csv", row.names = F)
...
#plsr-narea
sum(is.na(pn$corg))#11
sum(pn$corg< -1 | pn$corg > 1, na.rm = T) #80
sum(pn$corgblup< -1 | pn$corgblup > 1, na.rm = T)  #3210
pn <- pn |>
  mutate(across(coh2:vartrait, ~replace(., which(corgblup < -1 |
                                                   corgblup > 1 |
                                                   corg < -1 |
...
sel$rowname <- paste0(sel$wave_1, "_", sel$wave_2)
pnratios <- wave |> select(wave_1081_wave_389:wave_1382_wave_2338)
pnratios <- t(pnratios)
...
write.csv(pnratios, "./output/pnratios.csv", row.names= F)
#absolute difference between wave_1 and wave_2
pnratios <- pnratios %>%
  mutate(diff = abs(wave_1 - wave_2))
...
pnratio_transform <- pnratio_sel %>%
  mutate(across(c(wave_1, wave_2), ~paste("wave", ., sep = "_"))) %>%
  mutate(wave_sel= paste(wave_1, wave_2, sep = "_")) %>% select(-group,-coh2,-diff)
write.csv(pnratio_transform, "./output/pnratio_transform.csv", row.names=F)
pnratio_transform<- read.csv("./output/pnratio_transform.csv")
...
psratio_transform <- psratio_sel %>%
  mutate(across(c(wave_1, wave_2), ~paste("wave", ., sep = "_"))) %>%
  mutate(wave_sel= paste(wave_1, wave_2, sep = "_")) %>% select(-group,-coh2,-diff)
write.csv(psratio_transform, "./output/psratio_transform.csv", row.names=F)
psratio_transform<- read.csv("./output/psratio_transform.csv")
