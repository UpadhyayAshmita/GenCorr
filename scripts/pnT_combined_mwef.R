...
run_trait_model <- function(trait, trait_index, cv_scheme) {
  ac_rep <- c()
  fname <- paste0("pnT", trait_index, "_", cv_scheme, "_rep", repetition, "_mwef.txt")
  ac_fname <- paste0("acpnT", trait_index, "_", cv_scheme, "_rep", repetition, "_mwef.txt")
  ...
  for (j in 1:length(sort)) {
    ...
    raS <- raS %>% left_join(pn_bluesEF[, c("taxa", "pn")])
