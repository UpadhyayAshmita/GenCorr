...
# Ensuring we follow the "kinship_additive" requirement
...
# Mapping traits to their specific CSV files
...
# JOIN with EF observations
raS <- bind_rows(r_rep) %>%
  left_join(data_EF[, c("taxa", trait_name)], by = "taxa") %>%
...
if (length(all_acc_files) == 20) {
  cat("\nAll reps finished. Merging results for:", trait_name, "\n")

  # Final Accuracy
  final_acc <- lapply(all_acc_files, fread) %>% bind_rows() %>% arrange(rep)
  fwrite(final_acc, file.path(out_dir, paste0("acc_mt_", trait_name, "_MWEF_", cv_scheme, ".csv")))
  ...
  fwrite(final_preds, file.path(out_dir, paste0("MT_", trait_name, "_MWEF_", cv_scheme, "_FINAL_preds.csv")))

  unlink(tmp_dir, recursive = TRUE)
  cat("Success. TMP cleaned.\n")
}
