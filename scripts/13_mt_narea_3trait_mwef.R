...
# FLIPPED: MW is now the training set, EF is the validation set
narea_EF <- narea_data %>%
  filter(env == "EF") %>%
...
# --- 5. Get Starting Values (Now using MW data) ---
cat(sprintf("\nStep 1: Fitting full model on MW for %s (W1: %s, W2: %s)...\n", cv_scheme, w1_col, w2_col))
...
fname <- paste0("MT_Narea_3Trait_", cv_scheme, "_preds_mwef.csv")
ac_fname <- paste0("MT_Narea_3Trait_", cv_scheme, "_accuracy_mwef.csv")
...
for (j in 1:20) {
  ...
  for (i in 1:5) {
    ...
    test_data <- narea_MW # Training on MW
  ...
  # VALIDATION: Now joining with EF data to check accuracy
  raS <- bind_rows(r_rep) %>%
    left_join(narea_EF[, c("taxa", "narea")], by = "taxa") %>%
    rename(obs = narea)
