...
# --- 4. Load Phenotypes ---
# Using the file you specified with the columns provided
...
cat(sprintf("\nStep 1: Fitting full model for %s (plsr_narea + Waves)...\n", cv_scheme))
# Formula uses the actual column name: plsr_narea
...
# --- 7. Unified Loop ---
# Output files use 'pn' as requested
fname <- paste0("MT_pn_3Trait_", cv_scheme, "_preds.csv")
ac_fname <- paste0("MT_pn_3Trait_", cv_scheme, "_accuracy.csv")
...
for (j in 1:20) {
  ...
  for (i in 1:5) {
    ...
    # Predict trait:taxa and filter for plsr_narea
  ...
  # Join with MW observed values for plsr_narea
  raS <- bind_rows(r_rep) %>%
    left_join(pn_MW[, c("taxa", "plsr_narea")], by = "taxa") %>%
