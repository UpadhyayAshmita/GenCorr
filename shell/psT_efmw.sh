#!/bin/bash
#SBATCH --job-name=psT_mwef
#SBATCH --output=logs/ppsT_mwef%A_%a.out
#SBATCH --error=logs/ST_mwef%A_%a.err
...
Rscript scripts/psT_combined.R --trait "$trait" --repetition "$rep_index" --cv_scheme "$cv_scheme"
