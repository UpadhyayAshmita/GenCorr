#!/bin/bash
#SBATCH --job-name=narea_completemwefcomb
#SBATCH --output=logs/narea_completemwefcomb_%A_%a.out
#SBATCH --error=logs/narea_completemwefcomb_%A_%a.err
...
Rscript scripts/narea_completemodel_mwef.R --trait "$trait" --trait_index "$trait_index" --cv_scheme "$cv_scheme"
