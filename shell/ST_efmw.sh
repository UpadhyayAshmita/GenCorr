#!/bin/bash
#SBATCH --job-name=ST_efmw
#SBATCH --output=logs/ST_%A_%a.out  # %A for array job ID, %a for array task ID
#SBATCH --error=logs/ST_%A_%a.err   # %A for array job ID, %a for array task ID
...
#SBATCH --time=20:00:00             # Set a time limit
...
Rscript scripts/ST_combined.R --trait "$trait" --repetition "$rep_index" --cv_scheme "$cv_scheme"
